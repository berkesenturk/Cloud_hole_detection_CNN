...
class NetCDFToZarrConverter:
    """
    Converts NetCDF files to Zarr format optimized for cloud storage and ML training
    """
    ...
    def analyze_netcdf_files(self, file_pattern: str) -> Dict:
        ...
        Args:
            file_pattern: Glob pattern for NetCDF files (e.g., "data/hrv_lr*.nc")
        ...
        analysis = {
            ...
            'recommended_chunks': self._calculate_optimal_chunks(dims, dtype_size)
        }
    ...
    # def _calculate_optimal_chunks(self, dims: Dict, dtype_size: int) -> Dict:
    #     """
    #     Calculate optimal chunk sizes for cloud storage (target: 10-50MB per chunk)
    #     """
    #     target_chunk_bytes = 20 * 1024 * 1024  # 20MB target
    ...
    #         if dim in dims:
    #             chunks[dim] = min(self.chunk_spatial, dims[dim])
    ...
    #     chunk_elements = np.prod(list(chunks.values()))
    #     chunk_mb = (chunk_elements * dtype_size) / (1024 * 1024)
        
    #     logger.info(f"Calculated chunk size: {chunk_mb:.2f} MB")
    ...
    def _calculate_optimal_chunks(self, dims: Dict, dtype_size: int) -> Dict:
        ...
        lat_dim = next((d for d in ['lat', 'latitude', 'y'] if d in dims), None)
        lon_dim = next((d for d in ['lon', 'longitude', 'x'] if d in dims), None)

        if not (time_dim and lat_dim and lon_dim):
            raise ValueError("Expected time, lat/y, lon/x dimensions for Zarr chunking")
        ...
        if bytes_per_timestep == 0:
            raise ValueError("Invalid dimension sizes for chunk calculation")
        ...
        logger.info(f"Calculated chunk size: {chunk_mb:.2f} MB | chunks={chunks}")
    ...
    def convert_multiple_files_to_single_zarr(
        ...
        This is recommended for time-series data across multiple months/years
        
        Args:
            file_pattern: Glob pattern for NetCDF files
            output_path: Path for output Zarr store
            time_dim: Name of time dimension for concatenation
            custom_chunks: Optional custom chunking dictionary
        """
        logger.info(f"Converting multiple files to single Zarr: {file_pattern}")
        ...
        try:
            ...
            logger.info(f"Writing consolidated Zarr with chunks: {chunks}")
            ...
            ds.close()
            logger.info(f"Successfully created consolidated Zarr at {output_path}")
    ...
    def convert_files_individually(
        ...
        Use this if you want to maintain file-level granularity
    ...
    def _get_encoding(self, ds: xr.Dataset, chunks: Dict) -> Dict:
        ...
        for var in ds.data_vars:
            # Use Blosc compression (fast and efficient)
            encoding[var] = {
                'compressor': zarr.Blosc(
                    cname='zstd',  # zstd is fast and efficient
